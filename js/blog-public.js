document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    grid.innerHTML = result.documents.map((post) => `<a href="blog-post.html?id=${encodeURIComponent(post.$id)}" class="blog-card reveal"><div class="blog-thumbnail"><div class="blog-thumbnail-bg bg-1"></div><div class="blog-date-badge"><span class="day">${datePart(post.date, 'day')}</span><span class="month">${datePart(post.date, 'month')}</span></div></div><div class="blog-body"><span class="badge badge-blue">${escapeHtml(post.category || 'Insights')}</span><h3>${escapeHtml(post.title)}</h3><p>${escapeHtml(post.excerpt || '')}</p><div class="blog-meta"><span><i data-lucide="user"></i> ${escapeHtml(post.author || 'TREEZ AFRICA SYSTEMS')}</span><span><i data-lucide="clock"></i> ${escapeHtml(post.readTime || '')}</span></div></div></a>`).join('');
